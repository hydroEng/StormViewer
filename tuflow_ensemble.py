...
def get_all_max_flows(po_df: pd.DataFrame) -> pd.Series:
    """
...
def concat_po_srs(max_flows_dfs: list[pd.Series]) -> pd.DataFrame:
    """
    This function reads a list of pd.Series containing dataframes with max flows for each run configuration.
    Only pass list generated by get_all_max_flows().
...
def drop_sort_duration(df: pd.DataFrame) -> pd.DataFrame:
    sorted_df = df
    """
...
def tp_vs_max_flow_df(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    dur_tp_df = drop_sort_duration(dur_tp_df)
...
def all_critical_storms(all_runs_df: pd.DataFrame) -> list[pd.DataFrame]:
    ...
    durations, as well as the critical storms for each duration. The input must be a dataframe from tp_vs_max_flow_df().
    ...
    Returns:
        A DataFrame selecting the duration with the highest
    ...
    for x in working_dfs:
        for y in x:
            ...
            all_crit_tp_dfs.append(drop_sort_duration(df))
...
def main(input_path: str):
    ...
    for csv in raw_inputs:
        ...
        sr1 = get_all_max_flows(df1)
        print(sr1)
        all_max_flows.append(sr1)

    combined_max_flows = concat_po_srs(all_max_flows)

    # for x in all_max_flows:
        # print(x)
